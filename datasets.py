...
class TextLineDataset(BaseDataset):
    ...
    def __or__(self, other: BaseDataset):
        return list(set(self.data).union(set(other.data)))

    def __and__(self, other: BaseDataset):
        return list(set(self.data).intersection(set(other.data)))

    def __sub__(self, other: BaseDataset):
...
class KeyValueDataset(BaseDataset):
    ...
    def __init__(self, data: List[dict], task_type=TaskType.TEXT_TO_TEXT_GENERATION, fields: List[str]=None):
        ...
        if self.task_type == TaskType.TEXT_TO_TEXT_GENERATION:
            if len(self.fields) == 2:
                ...
                self.operation_type = "sentenceandtarget"
    ...
    def _apply_sentenceandtarget_filter(self, datapoint: dict, filter: SentenceAndTargetOperation):
        sentence = datapoint[self.fields[0]]
        target = datapoint[self.fields[1]]
        return filter.filter(sentence, target)
    
    def _apply_sentenceandtargets_filter(self, datapoint: dict, filter: SentenceAndTargetsOperation):
        sentence = datapoint[self.fields[0]]
        targets = [datapoint[target_key] for target_key in self.fields[1:]]
        return filter.filter(sentence, targets)
    
    def _apply_questionanswer_filter(self, datapoint: dict, filter: QuestionAnswerOperation):
    ...
    def _apply_sentenceandtarget_transformation(self, datapoint: dict, transformation: SentenceAndTargetOperation):
        sentence = datapoint[self.fields[0]]
        target = datapoint[self.fields[1]]
    ...
    def _apply_sentenceandtargets_transformation(self, datapoint: dict, transformation: SentenceAndTargetsOperation):
        sentence = datapoint[self.fields[0]]
        targets = [datapoint[target_key] for target_key in self.fields[1:]]
    ...
    def _apply_questionanswer_transformation(self, datapoint: dict, transformation: QuestionAnswerOperation):
        context = datapoint[self.fields[0]]
        question = datapoint[self.fields[1]]
        answers = [datapoint[answer_key] for answer_key in self.fields[2:]]
    ...
    def _sanity_check(self, other: BaseDataset):
        ...
        assert self.fields == other.fields, "You cannot do dataset operation on datasets with different fields"
