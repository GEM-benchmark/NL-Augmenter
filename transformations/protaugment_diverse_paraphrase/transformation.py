...
class UnigramForbidStrategy(ForbidStrategy):
    ...
    def bad_words_ids(self, input_ids: torch.Tensor, special_ids: List[str] = None) -> List[List[str]]:
        bad_words_ids = list()
...
class ProtaugmentDiverseParaphrase(SentenceOperation):
    ...
    def __init__(
        ...
        assert num_beams % beam_group_size == 0  # `num_beams` should be a multiple of `beam_group_size`
    ...
    def prepare_batch(self, sentence: str) -> Dict[str, Union[torch.Tensor, List[List[str]]]]:
        batch = self.tokenizer.prepare_seq2seq_batch(src_texts=[sentence], return_tensors="pt", max_length=512)
    ...
    def generate(self, sentence: str):
        ...
        filtered = filter_generated_texts_with_distance_metric(
            texts=[tgt_texts[i:i + self.beam_group_size] for i in range(0, len(tgt_texts), self.beam_group_size)],
            src=sentence,
