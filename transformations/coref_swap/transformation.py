...
def swap_coref(sent, nlp, verbose=False, how_likely=3):
    ...
    for cluster_id, coref_cluster in enumerate(doc._.coref_clusters):
        ...
        for i, j in swap_pairs:
            ...
            mention_i_text_full = sent[doc[mention_i.start].idx:doc[mention_i.end - 1].idx + len(
                doc[mention_i.end - 1].text)]
            mention_j_text_full = sent[doc[mention_j.start].idx:doc[mention_j.end - 1].idx + len(
                doc[mention_j.end - 1].text)]

            mention_i_replacer = {"start": doc[mention_i.start].idx,
                                  "end": doc[mention_i.end - 1].idx + len(doc[mention_i.end - 1].text),
                                  "repn": mention_j_text_full, "start_replacer": doc[mention_j.start].idx,
                                  "end_replacer": doc[mention_j.end - 1].idx + len(doc[mention_j.end - 1].text)}
    ...
    for mention_replacer_id in range(len(mention_replacers) - 1):
        ...
        new_string += sent[
                      mention_replacers[mention_replacer_id]["end"]:mention_replacers[mention_replacer_id + 1]["start"]]
    if len(mention_replacers) > 0:
