...
class MixTransliteration(SentenceOperation):
    ...
    def __init__(self, source_lang: str, target_lang: Optional[str] = "en", seed: Optional[int] = 42):
        super().__init__()
        random.seed(seed)
        assert source_lang in self.LANG_MAPPING.keys(), NotImplementedError(f"Incorrect source language, choose one of {list(self.LANG_MAPPING.keys())}")
        assert target_lang in self.LANG_MAPPING.keys(), NotImplementedError(f"Incorrect target language, choose one of {list(self.LANG_MAPPING.keys())}")
        print(f"Transformation initialized from {self.LANG_MAPPING[source_lang]} to {self.LANG_MAPPING[target_lang]}")
    ...
    def generate(self, sentence: str, prob_mix: int = 1):
        temp_text = transliterate(sentence, source=self.source_lang, target=self.target_lang, universal=True)
