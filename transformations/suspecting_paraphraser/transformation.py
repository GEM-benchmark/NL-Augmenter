...
class SuspectingParaphraser(QuestionAnswerOperation):
    ...
    def _transform(self, question):
        ...
        verb_position = [i for i in range(len(doc)) if str(doc[i])==token.head.text][0]

        rest_of_sentence = [i for i in text[verb_position:]]
        beginning = [text[1].capitalize()]
        beginning.extend(text[2:verb_position])
        sentence = nltk.tokenize.treebank.TreebankWordDetokenizer().detokenize(beginning + rest_of_sentence)
    ...
    def generate(
        ...
        question = self._transform(question)
...
if __name__ == '__main__':
    ...
    test_cases = []
    for i, sentence in enumerate(["Did Sally finally return the french book to Chris?",
                                "Did the American National Shipment company really break its own fleet?",
                                "Would you be so kind to pass me the soup?",
        ...
        test_cases.append({
            "class": tf.name(),
            "inputs": {"context": "", "question": sentence, "answers": []}, "outputs": []}
        )

        for p_context, p_question, p_answers in res:
            test_cases[i]["outputs"].append({"context": p_context, "question": p_question, "answers": p_answers})
