# -*- coding: utf-8 -*-

import random

from typing import List
from checklist.editor import Editor

from interfaces.SentenceOperation import SentenceOperation
from tasks.TaskTypes import TaskType
#TOP 1000 MOST COMMON GERMAN MALE AND FEMALE NAMES,
names={1: {0: 'Mary',
  1: 'Ursula',
  2: 'Monika',
  3: 'Petra',
  4: 'Elisabeth',
  5: 'Sabine',
  6: 'Renate',
  7: 'Helga',
  8: 'Karin',
  9: 'Brigitte',
  10: 'Ingrid',
  11: 'Erika',
  12: 'Andrea',
  13: 'Gisela',
  14: 'Claudia',
  15: 'Susanne',
  16: 'Gabriele',
  17: 'Christa',
  18: 'Christine',
  19: 'Hildegard',
  20: 'Anna',
  21: 'Birgit',
  22: 'Barbara',
  23: 'Gertrude',
  24: 'Heike',
  25: 'Marianne',
  26: 'Elke',
  27: 'Martina',
  28: 'Angelica',
  29: 'Irmgard',
  30: 'Inge',
  31: 'Ute',
  32: 'Elfriede',
  33: 'Doris',
  34: 'Marion',
  35: 'Ruth',
  36: 'Ulrike',
  37: 'Hannelore',
  38: 'Jutta',
  39: 'Gerda',
  40: 'Kerstin',
  41: 'Ilse',
  42: 'Anneliese',
  43: 'Margarete',
  44: 'Ingeborg',
  45: 'Anja',
  46: 'Edith',
  47: 'Sandra',
  48: 'Waltraud',
  49: 'Beate',
  50: 'Rita',
  51: 'Catherine',
  52: 'Christel',
  53: 'Nicole',
  54: 'Regina',
  55: 'Eva',
  56: 'Rosemary',
  57: 'Erna',
  58: 'Manuela',
  59: 'Sonja',
  60: 'Johanna',
  61: 'Irene',
  62: 'Silke',
  63: 'Gudrun',
  64: 'Christiane',
  65: 'Cornelia',
  66: 'Tanja',
  67: 'Anita',
  68: 'Bettina',
  69: 'Silvia',
  70: 'Daniela',
  71: 'Sigrid',
  72: 'Simone',
  73: 'Stefanie',
  74: 'Annette',
  75: 'Bärbel',
  76: 'Michaela',
  77: 'Angela',
  78: 'Dagmar',
  79: 'Heidi',
  80: 'Annemarie',
  81: 'Helene',
  82: 'Anke',
  83: 'Margot',
  84: 'Sylvia',
  85: 'Christina',
  86: 'Katrin',
  87: 'Melanie',
  88: 'Hedwig',
  89: 'Roswitha',
  90: 'Martha',
  91: 'Alexandra',
  92: 'Else',
  93: 'Iris',
  94: 'Katja',
  95: 'Charlotte',
  96: 'Lieselotte',
  97: 'Hilde',
  98: 'Astrid',
  99: 'Anni',
  100: 'Margit',
  101: 'Frieda',
  102: 'Carmen',
  103: 'Anne',
  104: 'Ilona',
  105: 'Louise',
  106: 'Margret',
  107: 'Dorothea',
  108: 'Pink',
  109: 'Herta',
  110: 'Olga',
  111: 'Lydia',
  112: 'Julia',
  113: 'Marlies',
  114: 'Yvonne',
  115: 'Antje',
  116: 'Kathe',
  117: 'Kathrin',
  118: 'Agnes',
  119: 'Gerlinde',
  120: 'Irma',
  121: 'Vera',
  122: 'Edeltraud',
  123: 'Ines',
  124: 'Stephanie',
  125: 'Carola',
  126: 'Franziska',
  127: 'Heidrun',
  128: 'Marina',
  129: 'Britta',
  130: 'Nadine',
  131: 'Diana',
  132: 'Ellen',
  133: 'Elvira',
  134: 'Sieglinde',
  135: 'Gabi',
  136: 'Emma',
  137: 'Veronica',
  138: 'Marita',
  139: 'Theresia',
  140: 'Bianca',
  141: 'Clare',
  142: 'Kirsten',
  143: 'Magdalena',
  144: 'Adelheid',
  145: 'Annegret',
  146: 'Brunhilde',
  147: 'Nina',
  148: 'Ina',
  149: 'Hannah',
  150: 'Uta',
  151: 'Heidemarie',
  152: 'Therese',
  153: 'Gertraud',
  154: 'Ramona',
  155: 'Lisa',
  156: 'Marga',
  157: 'Paula',
  158: 'Marlene',
  159: 'Irina',
  160: 'Berta',
  161: 'Tatiana',
  162: 'Verena',
  163: 'Marie',
  164: 'Anette',
  165: 'Evelyn',
  166: 'Jana',
  167: 'Elena',
  168: 'Elli',
  169: 'Mathilde',
  170: 'Eva-Maria',
  171: 'Corinna',
  172: 'Wilma',
  173: 'Lore',
  174: 'Eleanor',
  175: 'Judith',
  176: 'Sabrina',
  177: 'Gaby',
  178: 'Karola',
  179: 'Liselotte',
  180: 'Elsa',
  181: 'Elsbeth',
  182: 'Isolde',
  183: 'Ida',
  184: 'Heather',
  185: 'Ella',
  186: 'Tina',
  187: 'Thea',
  188: 'Roger',
  189: 'Dora',
  190: 'Sibyl',
  191: 'Vine',
  192: 'Cornelia',
  193: 'Mechthild',
  194: 'Waltraut',
  195: 'Lotte',
  196: 'Sybille',
  197: 'Dorothee',
  198: 'Joseph',
  199: 'Annett',
  200: 'Steffi',
  201: 'Margarethe',
  202: 'Gabriela',
  203: 'Maren',
  204: 'Patricia',
  205: 'Marie-Luise',
  206: 'Juliane',
  207: 'Pia',
  208: 'Valentina',
  209: 'Hermione',
  210: 'Lina',
  211: 'Ingeburg',
  212: 'Brigitta',
  213: 'Jessica',
  214: 'Karla',
  215: 'Grete',
  216: 'Frauke',
  217: 'Helena',
  218: 'Margrit',
  219: 'Alice',
  220: 'Magdalene',
  221: 'Kristina',
  222: 'Beatrix',
  223: 'Jennifer',
  224: 'Hella',
  225: 'Miriam',
  226: 'Walburga',
  227: 'Friederike',
  228: 'Maria',
  229: 'Marlis',
  230: 'Natalie',
  231: 'Eveline',
  232: 'Nadja',
  233: 'Sarah',
  234: 'Emilie',
  235: 'Meike',
  236: 'Edelgard',
  237: 'Emmi',
  238: 'Doreen',
  239: 'Ulla',
  240: 'Sophie',
  241: 'Ursel',
  242: 'Esther',
  243: 'Ilka',
  244: 'Martha',
  245: 'Viola',
  246: 'Jasmine',
  247: 'Helmet',
  248: 'Linda',
  249: 'Mandy',
  250: 'Natalia',
  251: 'Edda',
  252: 'Carolin',
  253: 'Wilhelmine',
  254: 'Annelies',
  255: 'Hilda',
  256: 'Jean',
  257: 'Lilli',
  258: 'Cordula',
  259: 'Lucia',
  260: 'Karen',
  261: 'Hiltrud',
  262: 'Lidia',
  263: 'Tamara',
  264: 'Annerose',
  265: 'Caroline',
  266: 'Margaret',
  267: 'Peter',
  268: 'Edeltraut',
  269: 'Kate',
  270: 'Karina',
  271: 'Antonia',
  272: 'Jean',
  273: 'Peter',
  274: 'Evelin',
  275: 'Jacqueline',
  276: 'Victoria',
  277: 'Janine',
  278: 'Annelie',
  279: 'Hertha',
  280: 'Liesel',
  281: 'Rosi',
  282: 'Inga',
  283: 'Magda',
  284: 'Grit',
  285: 'Gunda',
  286: 'Lucie',
  287: 'Joseph',
  288: 'Alma',
  289: 'Caroline',
  290: 'Meta',
  291: 'Henriette',
  292: 'Rosemary',
  293: 'Wiebke',
  294: 'Uschi',
  295: 'Peter',
  296: 'Peggy',
  297: 'Adele',
  298: 'Natasha',
  299: 'Elly',
  300: 'Sigrun',
  301: 'Elise',
  302: 'Dorte',
  303: 'Goddess',
  304: 'Kristin',
  305: 'Jeanette',
  306: 'Antonie',
  307: 'Gretel',
  308: 'Betty',
  309: 'Isabel',
  310: 'Lena',
  311: 'Constanze',
  312: 'Jenny',
  313: 'Isabella',
  314: 'Carina',
  315: 'Gundula',
  316: 'Elfi',
  317: 'Rosel',
  318: 'Janina',
  319: 'Galina',
  320: 'Gitta',
  321: 'Susan',
  322: 'Denise',
  323: 'Traute',
  324: 'Evi',
  325: 'Beatrice',
  326: 'Annika',
  327: 'Anna- maria',
  328: 'Jean',
  329: 'Svetlana',
  330: 'Wendelin',
  331: 'Larissa',
  332: 'Leni',
  333: 'Svetlana',
  334: 'Sophie',
  335: 'Sylke',
  336: 'Bianka',
  337: 'Carla',
  338: 'Resi',
  339: 'Ludmilla',
  340: 'Siegrid',
  341: 'Pauline',
  342: 'Frida',
  343: 'Vanessa',
  344: 'Rosina',
  345: 'Dorit',
  346: 'Anett',
  347: 'Melitta',
  348: 'Auguste',
  349: 'Mareike',
  350: 'Bernadette',
  351: 'Annelore',
  352: 'Jenny',
  353: 'Ria',
  354: 'Irena',
  355: 'Peter',
  356: 'Laura',
  357: 'Klaudia',
  358: 'Babette',
  359: 'Sofia',
  360: 'Lisbeth',
  361: 'Almut',
  362: 'Susanna',
  363: 'Wanda',
  364: 'Svenja',
  365: 'Jean',
  366: 'Emmy',
  367: 'Kathleen',
  368: 'Fatma',
  369: 'Lucia',
  370: 'Nelli',
  371: 'Christl',
  372: 'Ariane',
  373: 'Jeannette',
  374: 'Corina',
  375: 'Saskia',
  376: 'Rebecca',
  377: 'Isabell',
  378: 'Wally',
  379: 'Hanne',
  380: 'Thekla',
  381: 'Carolina',
  382: 'Felicitas',
  383: 'Cecilia',
  384: 'Valeri',
  385: 'Conny',
  386: 'Jean',
  387: 'Selma',
  388: 'Birte',
  389: 'Marliese',
  390: 'Lucie',
  391: 'Peter',
  392: 'Romy',
  393: 'Anny',
  394: 'Agathe',
  395: 'Nora',
  396: 'Kirstin',
  397: 'Cecilia',
  398: 'Jean',
  399: 'Dana',
  400: 'Teresa',
  401: 'Marika',
  402: 'Nancy',
  403: 'Alwine',
  404: 'Amalie',
  405: 'Lilo',
  406: 'Imke',
  407: 'Margarita',
  408: 'Johannes',
  409: 'Henny',
  410: 'Margaretha',
  411: 'Mary',
  412: 'Christian',
  413: 'Rose',
  414: 'Gertrude',
  415: 'Anika',
  416: 'Ricarda',
  417: 'Miriam',
  418: 'Gerti',
  419: 'Danuta',
  420: 'Sabina',
  421: 'Ayse',
  422: 'Maritta',
  423: 'Ludmila',
  424: 'Sina',
  425: 'Loni',
  426: 'Isabelle',
  427: 'Marija',
  428: 'Theresa',
  429: 'Nathalie',
  430: 'Eugenie',
  431: 'Sandy',
  432: 'Sophia',
  433: 'Monique',
  434: 'Minna',
  435: 'Marietta',
  436: 'Leonore',
  437: 'Ewa',
  438: 'Reinhild',
  439: 'Pamela',
  440: 'Reinhilde',
  441: 'Isa',
  442: 'Gerta',
  443: 'Mina',
  444: 'Trude',
  445: 'Henri',
  446: 'Janet',
  447: 'Natalia',
  448: 'Kunigunde',
  449: 'Wiltrud',
  450: 'Sara',
  451: 'Celeb',
  452: 'Emine',
  453: 'Gesa',
  454: 'Anne',
  455: 'Ortrud',
  456: 'Heather rose',
  457: 'Hatice',
  458: 'Silvana',
  459: 'Ana',
  460: 'Mona',
  461: 'Birgitta',
  462: 'Brunhild',
  463: 'Ernestine',
  464: 'Aloisia',
  465: 'Lilly',
  466: 'Halina',
  467: 'Valerie',
  468: 'Constanze',
  469: 'Monica',
  470: 'Joanna',
  471: 'Emilia',
  472: 'Rosalinde',
  473: 'Katarina',
  474: 'Evelyne',
  475: 'Celebs',
  476: 'Cindy',
  477: 'Katie',
  478: 'Catrin',
  479: 'Inca',
  480: 'Rosita',
  481: 'Jacqueline',
  482: 'Dune',
  483: 'Simona',
  484: 'Madeleine',
  485: 'Jolanta',
  486: 'Celebs',
  487: 'Helen',
  488: 'Jean',
  489: 'Cathrin',
  490: 'Inna',
  491: 'Marlen',
  492: 'Mary',
  493: 'Mania',
  494: 'Patricia',
  495: 'Centa',
  496: 'Traudel',
  497: 'Wera',
  498: 'Anastasia',
  499: 'Renata',
  500: 'Ira',
  501: 'Berit',
  502: 'Zita',
  503: 'Rosalia',
  504: 'Leon',
  505: 'Grazyna',
  506: 'Anne-Marie',
  507: 'Diane',
  508: 'Eugenia',
  509: 'Amalia',
  510: 'Alina',
  511: 'Brita',
  512: 'Nelly',
  513: 'Henrike',
  514: 'Mira',
  515: 'Walli',
  516: 'Mechtild',
  517: 'Amanda',
  518: 'Laura',
  519: 'Christiana',
  520: 'Kristine',
  521: 'Victoria',
  522: 'Dina',
  523: 'Traude',
  524: 'Myriam',
  525: 'Almuth',
  526: 'Solveig',
  527: 'Alla',
  528: 'Lioba',
  529: 'Juliana',
  530: 'Rosalie',
  531: 'Undine',
  532: 'Gunhild',
  533: 'Dörthe',
  534: 'Editha',
  535: 'Bozena',
  536: 'Herma',
  537: 'Fanny',
  538: 'Clara',
  539: 'Reekka',
  540: 'Luitgard',
  541: 'Lydia',
  542: 'Julie',
  543: 'Annemie',
  544: 'Mary',
  545: 'Alexa',
  546: 'Paulina',
  547: 'Sibille',
  548: 'Heir',
  549: 'Beata',
  550: 'Catherine',
  551: 'Anka',
  552: 'Theodora',
  553: 'Jean',
  554: 'Bernard',
  555: 'Roswita',
  556: 'Aenne',
  557: 'Susi',
  558: 'Maria-Luise',
  559: 'Malgorzata',
  560: 'Edeltrud',
  561: 'Hedda',
  562: 'Kordula',
  563: 'Jean',
  564: 'Michelle',
  565: 'Dietlind',
  566: 'Marie',
  567: 'Oxana',
  568: 'Mara',
  569: 'Lotti',
  570: 'Olivia',
  571: 'Eleonora',
  572: 'Babe',
  573: 'Ekaterina',
  574: 'Anne-Kathrin',
  575: 'Nada',
  576: 'Angelina',
  577: 'Madlen',
  578: 'Rotraud',
  579: 'Mariola',
  580: 'Gretchen',
  581: 'Jean',
  582: 'Gloria',
  583: 'Stella',
  584: 'Jelena',
  585: 'Louise',
  586: 'Greta',
  587: 'Magarete',
  588: 'Fatima',
  589: 'Catharina',
  590: 'Tatiana',
  591: 'Irmela',
  592: 'Dorothe',
  593: 'Gina',
  594: 'Insa',
  595: 'Aline',
  596: 'Elzbieta',
  597: 'Lea',
  598: 'Romana',
  599: 'Gisa',
  600: 'Senta',
  601: 'Dolores',
  602: 'Marieluise',
  603: 'Luisa',
  604: 'Betina',
  605: 'Kreszenz',
  606: 'Dorle',
  607: 'Josephine',
  608: 'Desiree',
  609: 'Annedore',
  610: 'Olav',
  611: 'Mia',
  612: 'Inks',
  613: 'Mechthilde',
  614: 'Cilli',
  615: 'Violetta',
  616: 'Nadia',
  617: 'Janette',
  618: 'Iwona',
  619: 'Eleni',
  620: 'Dorota',
  621: 'Gundi',
  622: 'Henni',
  623: 'Annelise',
  624: 'Belinda',
  625: 'Mirjana',
  626: 'Siegrun',
  627: 'Georgia',
  628: 'Clear',
  629: 'Elisa',
  630: 'Jean',
  631: 'Rose-marie',
  632: 'Vesna',
  633: 'Adriana',
  634: 'Milena',
  635: 'Jane',
  636: 'Friedl',
  637: 'Rena',
  638: 'Norma',
  639: 'Anne-Katrin',
  640: 'Hannah',
  641: 'Goddess',
  642: 'Elfie',
  643: 'Tabea',
  644: 'Aurelia',
  645: 'Alena',
  646: 'Margita',
  647: 'Delia',
  648: 'Jeannine',
  649: 'Asta',
  650: 'Irmengard',
  651: 'Anneli',
  652: 'Karolin',
  653: 'Hulya',
  654: 'Valeria',
  655: 'Woodland',
  656: 'Ilonka',
  657: 'Dragica',
  658: 'Jean',
  659: 'Gordana',
  660: 'Elizabeth',
  661: 'Peter',
  662: 'Cilly',
  663: 'Swantje',
  664: 'Urte',
  665: 'Jean',
  666: 'Louis',
  667: 'Freya',
  668: 'Irmi',
  669: 'Marianna',
  670: 'Peter',
  671: 'Cristina',
  672: 'Bertha',
  673: 'Carmine',
  674: 'Yvette',
  675: 'Sevim',
  676: 'Zenta',
  677: 'Lilian',
  678: 'Mary',
  679: 'Miria',
  680: 'Cora',
  681: 'Yasmin',
  682: 'Jadwiga',
  683: 'Roger',
  684: 'Clarissa',
  685: 'Raphaela',
  686: 'Read',
  687: 'Birthe',
  688: 'Eileen',
  689: 'Wibke',
  690: 'Georgine',
  691: 'Leyla',
  692: 'Justine',
  693: 'Cathleen',
  694: 'Katarzyna',
  695: 'Lilija',
  696: 'Sonia',
  697: 'Franka',
  698: 'Aynur',
  699: 'Margaret',
  700: 'Yasemin',
  701: 'Ingetraud',
  702: 'Philomena',
  703: 'Aleksandra',
  704: 'Lilie',
  705: 'Liliane',
  706: 'Gilda',
  707: 'Carmel',
  708: 'Filiz',
  709: 'Britt',
  710: 'Gudula',
  711: 'Priska',
  712: 'Raisa',
  713: 'Albertine',
  714: 'Janett',
  715: 'Michaele',
  716: 'Hedy',
  717: 'Marie-theres',
  718: 'Camilla',
  719: 'Krescentia',
  720: 'Claire',
  721: 'Krista',
  722: 'Kunigunda',
  723: 'Carolina',
  724: 'Annelene',
  725: 'Olena',
  726: 'Ljubica',
  727: 'Gretl',
  728: 'Aysel',
  729: 'Xenia',
  730: 'Blanka',
  731: 'Janna',
  732: 'Holland',
  733: 'Suse',
  734: 'Apollonia',
  735: 'Benita',
  736: 'Virginia',
  737: 'Katherina',
  738: 'Gislinde',
  739: 'Monja',
  740: 'Slavica',
  741: 'Carin',
  742: 'Elizabeth',
  743: 'Mariana',
  744: 'Brit',
  745: 'Katy',
  746: 'Elif',
  747: 'Hildegunde',
  748: 'Meryem',
  749: 'Danica',
  750: 'Stupid',
  751: 'Rosewood',
  752: 'Irmtrud',
  753: 'Notburga',
  754: 'Freia',
  755: 'Flora',
  756: 'Rabea',
  757: 'Hildegart',
  758: 'Jasmina',
  759: 'Alice',
  760: 'Deborah',
  761: 'Nadin',
  762: 'Alwina',
  763: 'Chantal',
  764: 'Mariele',
  765: 'Hildburg',
  766: 'Mareen',
  767: 'Stefani',
  768: 'Reingard',
  769: 'Amelie',
  770: 'Maria Theresa',
  771: 'Goddess',
  772: 'Caren',
  773: 'Sylvie',
  774: 'Veronica',
  775: 'Maya',
  776: 'Harriet',
  777: 'Dajana',
  778: 'Francesca',
  779: 'Jo',
  780: 'Eva-Marie',
  781: 'Lidwina',
  782: 'Kira',
  783: 'Marisa',
  784: 'Hulda',
  785: 'Earthmute',
  786: 'Cecilia',
  787: 'Anica',
  788: 'Gusti',
  789: 'Gundel',
  790: 'Lia',
  791: 'Leopoldine',
  792: 'Rosl',
  793: 'Giovanna',
  794: 'Luca',
  795: 'Ivanka',
  796: 'Raissa',
  797: 'Birgid',
  798: 'Melita',
  799: 'Peter',
  800: 'Peter',
  801: 'Ellinor',
  802: 'Marzena',
  803: 'Lisette',
  804: 'Peter',
  805: 'Suzanne',
  806: 'Leokadia',
  807: 'Tania',
  808: 'Theda',
  809: 'Genoveva',
  810: 'Ayten',
  811: 'Annie',
  812: 'Sinaida',
  813: 'Marica',
  814: 'Stefania',
  815: 'Agatha',
  816: 'Rosa Maria',
  817: 'Francoise',
  818: 'Hanne-lore',
  819: 'Frederike',
  820: 'Adelinde',
  821: 'Valeska',
  822: 'Babett',
  823: 'Marie-louise',
  824: 'Zdenka',
  825: 'Justina',
  826: 'Katalin',
  827: 'Marcella',
  828: 'Dorina',
  829: 'Caterina',
  830: 'Jean',
  831: 'Heather marie',
  832: 'Angelique',
  833: 'Ruddy',
  834: 'Milka',
  835: 'Cosima',
  836: 'Lyudmila',
  837: 'Ann',
  838: 'Liliana',
  839: 'Constance',
  840: 'Danielle',
  841: 'Paola',
  842: 'Ortrun',
  843: 'Nermin',
  844: 'Gift',
  845: 'Sibylla',
  846: 'Ivana',
  847: 'Hannchen',
  848: 'Wenke',
  849: 'Tilly',
  850: 'Jolanthe',
  851: 'Annaliese',
  852: 'Meral',
  853: 'Theres',
  854: 'Erica',
  855: 'Clare',
  856: 'Jean',
  857: 'Gulay',
  858: 'Sibilla',
  859: 'Gabriella',
  860: 'Adina',
  861: 'Marjan',
  862: 'Ludwina',
  863: 'Jessika',
  864: 'Natali',
  865: 'Antoinette',
  866: 'Zofia',
  867: 'Franca',
  868: 'Corinne',
  869: 'Trudel',
  870: 'Ingetraut',
  871: 'Natalia',
  872: 'Lena',
  873: 'Hannie',
  874: 'Annekatrin',
  875: 'Kata',
  876: 'Arzu',
  877: 'Giuseppe',
  878: 'Irmingard',
  879: 'Adrian',
  880: 'Peter',
  881: 'Adolfine',
  882: 'Nuray',
  883: 'Nadezhda',
  884: 'Melissa',
  885: 'Ester',
  886: 'Adeline',
  887: 'Stilla',
  888: 'Lissy',
  889: 'Oda',
  890: 'Branka',
  891: 'Damaris',
  892: 'Serpil',
  893: 'Leila',
  894: 'Nadeshda',
  895: 'Martine',
  896: 'Ehrentraud',
  897: 'Christa-maria',
  898: 'Antonina',
  899: 'Bruni',
  900: 'Mirella',
  901: 'Songul',
  902: 'Luka',
  903: 'Milica',
  904: 'Geraldine',
  905: 'Tulay',
  906: 'Lara',
  907: 'Hilma',
  908: 'Evamaria',
  909: 'Marit',
  910: 'Edit',
  911: 'Cynthia',
  912: 'Susana',
  913: 'Pod',
  914: 'Semra',
  915: 'Concetta',
  916: 'Trudi',
  917: 'Sylvana',
  918: 'Anna-Luise',
  919: 'Agata',
  920: 'Gitte',
  921: 'Pascale',
  922: 'Lissi',
  923: 'Corinna',
  924: 'Alida',
  925: 'Zehra',
  926: 'Imelda',
  927: 'Sylwia',
  928: 'Donata',
  929: 'Florentine',
  930: 'Fabienne',
  931: 'Peter',
  932: 'Hanife',
  933: 'Peter',
  934: 'Viviane',
  935: 'Marie',
  936: 'Kathy',
  937: 'Joana',
  938: 'Florence',
  939: 'Jolanda',
  940: 'Ann-Kathrin',
  941: 'Antonietta',
  942: 'Annegrete',
  943: 'Aneta',
  944: 'Nurten',
  945: 'Anneke',
  946: 'Turkish',
  947: 'Elizabeth',
  948: 'Daria',
  949: 'Veronique',
  950: 'Mercedes',
  951: 'Geza',
  952: 'Fadime',
  953: 'Albina',
  954: 'Miroslava',
  955: 'Janin',
  956: 'Elma',
  957: 'Urszula',
  958: 'Polina',
  959: 'Katerina',
  960: 'Anna-Lena',
  961: 'Marie-Therese',
  962: 'Käthi',
  963: 'Ildiko',
  964: 'Telse',
  965: 'Ruthild',
  966: 'Nuran',
  967: 'Gonul',
  968: 'Angelica',
  969: 'Elwira',
  970: 'Elenore',
  971: 'Samira',
  972: 'Magrit',
  973: 'Lieschen',
  974: 'Ingried',
  975: 'Felicia',
  976: 'Friederike',
  977: 'Annamaria',
  978: 'Silva',
  979: 'August',
  980: 'Stupid',
  981: 'Svea',
  982: 'Anne-Rose',
  983: 'Valentine',
  984: 'Evangelia',
  985: 'Hello',
  986: 'Annita',
  987: 'Valentina',
  988: 'Sarina',
  989: 'Radmila',
  990: 'Laila',
  991: 'Canan',
  992: 'Anna-Marie',
  993: 'Etta',
  994: 'Sibel',
  995: 'Loretta',
  996: 'Marleen',
  997: 'Kristiane',
  998: 'Rachel',
  999: 'Sybilla'},
  2: {0: 'Peter',
  1: 'Wolfgang',
  2: 'Michael',
  3: 'Werner',
  4: 'Klaus',
  5: 'Thomas',
  6: 'Manfred',
  7: 'Helmut',
  8: 'Jürgen',
  9: 'Heinz',
  10: 'Gerhard',
  11: 'Andreas',
  12: 'Hans',
  13: 'Joseph',
  14: 'Günter',
  15: 'Dieter',
  16: 'Horst',
  17: 'Walter',
  18: 'Frank',
  19: 'Bernd',
  20: 'Karl',
  21: 'Herbert',
  22: 'Franz',
  23: 'Martin',
  24: 'Uwe',
  25: 'George',
  26: 'Heinrich',
  27: 'Stefan',
  28: 'Christian',
  29: 'Karl-Heinz',
  30: 'Rudolf',
  31: 'Kurt',
  32: 'Hermann',
  33: 'John',
  34: 'William',
  35: 'Siegfried',
  36: 'Rolf',
  37: 'Joachim',
  38: 'Alfred',
  39: 'Rainer',
  40: 'Joerg',
  41: 'Ralf',
  42: 'Erich',
  43: 'Norbert',
  44: 'Bernhard',
  45: 'Willi',
  46: 'Alexander',
  47: 'Ulrich',
  48: 'Mark',
  49: 'Matthias',
  50: 'Harald',
  51: 'Paul',
  52: 'Roland',
  53: 'Ernst',
  54: 'Reinhard',
  55: 'Günther',
  56: 'Gerd',
  57: 'Fritz',
  58: 'Otto',
  59: 'Frederick',
  60: 'Erwin',
  61: 'Lothar',
  62: 'Robert',
  63: 'Dirk',
  64: 'John',
  65: 'Volker',
  66: 'Wilfried',
  67: 'Richard',
  68: 'Anton',
  69: 'Jens',
  70: 'Hans-Jürgen',
  71: 'Hubert',
  72: 'Udo',
  73: 'Holger',
  74: 'Albert',
  75: 'Ludwig',
  76: 'Dietmar',
  77: 'Hartmut',
  78: 'Reinhold',
  79: 'Hans-Joachim',
  80: 'Adolf',
  81: 'Detlef',
  82: 'Oliver',
  83: 'Christopher',
  84: 'Stephan',
  85: 'Axel',
  86: 'Reiner',
  87: 'Alois',
  88: 'Eberhard',
  89: 'Waldemar',
  90: 'Heiko',
  91: 'Daniel',
  92: 'Torsten',
  93: 'Sven',
  94: 'Bruno',
  95: 'Olaf',
  96: 'Mario',
  97: 'Konrad',
  98: 'Steffen',
  99: 'Ingo',
  100: 'Jochen',
  101: 'Thorsten',
  102: 'Max',
  103: 'Alfons',
  104: 'Rüdiger',
  105: 'Carsten',
  106: 'Viktor',
  107: 'Hans-Peter',
  108: 'Rudi',
  109: 'Friedhelm',
  110: 'Armin',
  111: 'Jan',
  112: 'Lutz',
  113: 'Ewald',
  114: 'Winfried',
  115: 'Egon',
  116: 'Erhard',
  117: 'Sebastian',
  118: 'Jacob',
  119: 'Marco',
  120: 'Harry',
  121: 'Eduard',
  122: 'Eugen',
  123: 'Karlheinz',
  124: 'Andre',
  125: 'Klaus-Dieter',
  126: 'Achim',
  127: 'Karsten',
  128: 'Edgar',
  129: 'Claus',
  130: 'Hans-Dieter',
  131: 'Tobias',
  132: 'Theo',
  133: 'Mathias',
  134: 'Gottfried',
  135: 'Emil',
  136: 'Guido',
  137: 'Arno',
  138: 'Marc',
  139: 'Eckhard',
  140: 'Marcus',
  141: 'Gustav',
  142: 'Florian',
  143: 'Dietrich',
  144: 'Theodore',
  145: 'Berthold',
  146: 'Burkhard',
  147: 'Rene',
  148: 'Artur',
  149: 'August',
  150: 'Edmund',
  151: 'Arnold',
  152: 'Joseph',
  153: 'Lars',
  154: 'Patrick',
  155: 'Willy',
  156: 'Ferdinand',
  157: 'Gerald',
  158: 'Ralph',
  159: 'Hans-Georg',
  160: 'Fred',
  161: 'Leo',
  162: 'Bodo',
  163: 'Hugo',
  164: 'Philipp',
  165: 'Peter',
  166: 'Ronald',
  167: 'Maik',
  168: 'Oscar',
  169: 'Mike',
  170: 'David',
  171: 'Gert',
  172: 'Novel',
  173: 'Bjorn',
  174: 'Tungsten',
  175: 'Hans-Werner',
  176: 'Arthur',
  177: 'Gregor',
  178: 'Henning',
  179: 'Nicholas',
  180: 'Marcel',
  181: 'Felix',
  182: 'Elmar',
  183: 'Edwin',
  184: 'Gunter',
  185: 'Heribert',
  186: 'Leon',
  187: 'Adam',
  188: 'Raimund',
  189: 'Manuel',
  190: 'Albrecht',
  191: 'Clement',
  192: 'Ludger',
  193: 'Henry',
  194: 'Heiner',
  195: 'Ulf',
  196: 'Timo',
  197: 'Antonio',
  198: 'Lorenz',
  199: 'Benjamin',
  200: 'Detlev',
  201: 'Mark',
  202: 'Dennis',
  203: 'Oswald',
  204: 'Simon',
  205: 'Benno',
  206: 'Engelbert',
  207: 'Ronny',
  208: 'Mehmet',
  209: 'Xaver',
  210: 'Jorn',
  211: 'Ali',
  212: 'Gerold',
  213: 'Toni',
  214: 'Helmuth',
  215: 'Christof',
  216: 'Sergei',
  217: 'Volkmar',
  218: 'Mirko',
  219: 'Tim',
  220: 'André',
  221: 'Marko',
  222: 'Gernot',
  223: 'Boris',
  224: 'Hans-Jörg',
  225: 'Vladimir',
  226: 'Mustafa',
  227: 'Knut',
  228: 'Willibald',
  229: 'Celeb',
  230: 'Hans-Ulrich',
  231: 'Stefan',
  232: 'Hubertus',
  233: 'Giuseppe',
  234: 'Heinz-Dieter',
  235: 'Otmar',
  236: 'Heino',
  237: 'Enrico',
  238: 'Jean',
  239: 'Julius',
  240: 'Hendrik',
  241: 'Hans-Günter',
  242: 'Alwin',
  243: 'Heinz-Jürgen',
  244: 'Rene',
  245: 'Hartwig',
  246: 'Erik',
  247: 'Nils',
  248: 'Clemens',
  249: 'Vladimir',
  250: 'Helge',
  251: 'Joseph',
  252: 'Ivan',
  253: 'Roger',
  254: 'Victoria',
  255: 'Hilmar',
  256: 'Maximilian',
  257: 'Falk',
  258: 'Valentine',
  259: 'Gunther',
  260: 'Rupert',
  261: 'Hans- hermann',
  262: 'Arne',
  263: 'Goddess',
  264: 'Gunnar',
  265: 'Heinz',
  266: 'Hermann-Josef',
  267: 'Thilo',
  268: 'Adalbert',
  269: 'Alex',
  270: 'Tino',
  271: 'Andrej',
  272: 'Salvatore',
  273: 'Heinrich',
  274: 'Nico',
  275: 'Eric',
  276: 'Constantine',
  277: 'Ahmet',
  278: 'Joseph',
  279: 'Heinrich',
  280: 'Egbert',
  281: 'Fabian',
  282: 'Hasan',
  283: 'Ekkehard',
  284: 'Marian',
  285: 'Hansjörg',
  286: 'Christopher',
  287: 'Francesco',
  288: 'Hagen',
  289: 'Silvio',
  290: 'Wolf',
  291: 'Victoria',
  292: 'Nicholas',
  293: 'Klaus-Jürgen',
  294: 'John',
  295: 'Heinz- peter',
  296: 'Georgios',
  297: 'Bert',
  298: 'Benedict',
  299: 'Gisbert',
  300: 'Adrian',
  301: 'Leopold',
  302: 'Joseph',
  303: 'Giovanni',
  304: 'Eckart',
  305: 'Igor',
  306: 'Tilo',
  307: 'Ibrahim',
  308: 'Reimund',
  309: 'Aloys',
  310: 'Marek',
  311: 'Carl',
  312: 'Gebhard',
  313: 'Ullrich',
  314: 'Wolf-Dieter',
  315: 'Yuri',
  316: 'Gotthard',
  317: 'Huseyin',
  318: 'Hanni',
  319: 'Roberto',
  320: 'Bertram',
  321: 'Henrik',
  322: 'Hans-Gerd',
  323: 'Hans Peter',
  324: 'Hannes',
  325: 'Dimitrios',
  326: 'Albin',
  327: 'Hans Jürgen',
  328: 'Arnd',
  329: 'Hans-Otto',
  330: 'Arndt',
  331: 'Victor',
  332: 'Uli',
  333: 'Friedrich Wilhelm',
  334: 'Sonke',
  335: 'Horst-Dieter',
  336: 'Hans-Günther',
  337: 'Bastian',
  338: 'Vincenzo',
  339: 'Ismail',
  340: 'Norman',
  341: 'Heinz-Josef',
  342: 'Karl-Friedrich',
  343: 'Denis',
  344: 'Marius',
  345: 'Anthony',
  346: 'Frieder',
  347: 'Ansgar',
  348: 'Angelo',
  349: 'Tom',
  350: 'Rolf-Dieter',
  351: 'Harri',
  352: 'Painted',
  353: 'Rico',
  354: 'Goetz',
  355: 'Stefan',
  356: 'Hellmut',
  357: 'Bernard',
  358: 'Pascal',
  359: 'Falko',
  360: 'Fridolin',
  361: 'Anatoly',
  362: 'Milan',
  363: 'Eckard',
  364: 'Rafael',
  365: 'Moritz',
  366: 'Friedbert',
  367: 'Murat',
  368: 'January',
  369: 'Hardy',
  370: 'Sören',
  371: 'Hans-martin',
  372: 'Vitali',
  373: 'Jose',
  374: 'Heinz-Günter',
  375: 'Luigi',
  376: 'Julian',
  377: 'Henryk',
  378: 'Heinz-Werner',
  379: 'Klaus Dieter',
  380: 'Edward',
  381: 'Burghard',
  382: 'Ortwin',
  383: 'Hans-walter',
  384: 'Christos',
  385: 'Constantino',
  386: 'Pierre',
  387: 'Philip',
  388: 'Caspar',
  389: 'Kevin',
  390: 'Gabriel',
  391: 'Leonid',
  392: 'Hans-Wilhelm',
  393: 'Peter',
  394: 'Hanno',
  395: 'Kuno',
  396: 'Osman',
  397: 'Niels',
  398: 'Celeb',
  399: 'Dierk',
  400: 'Monday',
  401: 'Emanuel',
  402: 'Nikola',
  403: 'Fredi',
  404: 'Dean',
  405: 'Carlo',
  406: 'Claus-dieter',
  407: 'Guenter',
  408: 'Cornelius',
  409: 'Raphael',
  410: 'Peter',
  411: 'Gero',
  412: 'Andy',
  413: 'Mohamed',
  414: 'Torben',
  415: 'Domenico',
  416: 'Josip',
  417: 'Hans-J.',
  418: 'Celeb',
  419: 'Swen',
  420: 'Hawk',
  421: 'Till',
  422: 'Luke',
  423: 'Jean',
  424: 'Enno',
  425: 'Nicholas',
  426: 'Hans Dieter',
  427: 'Jens-Uwe',
  428: 'Michel',
  429: 'Franz Joseph',
  430: 'Vincent',
  431: 'Sigurd',
  432: 'Nicholas',
  433: 'Stanislaw',
  434: 'Goddess',
  435: 'Andree',
  436: 'Wulf',
  437: 'Diethard',
  438: 'Robin',
  439: 'Carlos',
  440: 'Veit',
  441: 'Franco',
  442: 'Heinz- george',
  443: 'Nicolas',
  444: 'Ivo',
  445: 'Peter',
  446: 'Danny',
  447: 'Jonas',
  448: 'Steven',
  449: 'Andrzej',
  450: 'Karl- otto',
  451: 'Walther',
  452: 'Klaus peter',
  453: 'Harro',
  454: 'Janusz',
  455: 'Hans-Christian',
  456: 'Sandro',
  457: 'Hans-jochen',
  458: 'Eckhardt',
  459: 'Jost',
  460: 'Karl-Wilhelm',
  461: 'Yusuf',
  462: 'Wenceslas',
  463: 'Bogdan',
  464: 'Peter',
  465: 'Gilbert',
  466: 'Ernst-August',
  467: 'Celibate',
  468: 'Ignaz',
  469: 'Wieland',
  470: 'Hans D.',
  471: 'Ludwig',
  472: 'Karl-Ernst',
  473: 'Hellmuth',
  474: 'Darius',
  475: 'Heinrich',
  476: 'Magnus',
  477: 'Helmar',
  478: 'Metin',
  479: 'Arnulf',
  480: 'Mirco',
  481: 'Juergen',
  482: 'Miroslaw',
  483: 'Sigmund',
  484: 'Claus- peter',
  485: 'Claudio',
  486: 'Peter',
  487: 'William',
  488: 'Heinz-joachim',
  489: 'Jonas',
  490: 'Halil',
  491: 'Ramazan',
  492: 'Ahmed',
  493: 'Hanns',
  494: 'Miroslav',
  495: 'Piotr',
  496: 'Peer',
  497: 'Helfried',
  498: 'Hans Georg',
  499: 'Samuel',
  500: 'Jerzy',
  501: 'Karl-Hermann',
  502: 'Kemal',
  503: 'Dragan',
  504: 'Mohammad',
  505: 'Heini',
  506: 'Hans-Willi',
  507: 'Jacek',
  508: 'Zbigniew',
  509: 'Rocco',
  510: 'Johannis',
  511: 'Joseph',
  512: 'Gordon',
  513: 'James',
  514: 'Bernward',
  515: 'Joerg',
  516: 'Bertold',
  517: 'Peter',
  518: 'Jean',
  519: 'Conrad',
  520: 'Francisco',
  521: 'Hasso',
  522: 'Zoran',
  523: 'Wolf-Rudiger',
  524: 'Matthew',
  525: 'Meik',
  526: 'Fernando',
  527: 'Peter',
  528: 'Jean',
  529: 'Sepp',
  530: 'Roy',
  531: 'Lambert',
  532: 'Guntram',
  533: 'Diedrich',
  534: 'Yilmaz',
  535: 'Damian',
  536: 'Constantin',
  537: 'Abdullah',
  538: 'Stanislaus',
  539: 'Ole',
  540: 'Andrei',
  541: 'Jean',
  542: 'Peter',
  543: 'Steve',
  544: 'Patrik',
  545: 'Jury',
  546: 'Hey',
  547: 'Cord',
  548: 'Ismet',
  549: 'Ivan',
  550: 'Niko',
  551: 'Tilman',
  552: 'Oemer',
  553: 'Peter',
  554: 'Raymond',
  555: 'Burkhardt',
  556: 'Pasquale',
  557: 'Nicolai',
  558: 'Kilian',
  559: 'Pius',
  560: 'Erol',
  561: 'Wolfhard',
  562: 'Branko',
  563: 'Adelbert',
  564: 'Ditmar',
  565: 'Paolo',
  566: 'Stephen',
  567: 'Jacob',
  568: 'Hans- michael',
  569: 'Valery',
  570: 'Ahmad',
  571: 'English',
  572: 'Dominic',
  573: 'Harm',
  574: 'Hajo',
  575: 'Stjepan',
  576: 'Zeljko',
  577: 'Hassan',
  578: 'Pavel',
  579: 'Patric',
  580: 'Orhan',
  581: 'Man',
  582: 'Reinhart',
  583: 'Laszlo',
  584: 'Hans-Herbert',
  585: 'Hans-helmut',
  586: 'Mohammed',
  587: 'Tadeusz',
  588: 'Claudius',
  589: 'Athanasios',
  590: 'Fredy',
  591: 'Luis',
  592: 'Roger',
  593: 'Andrew',
  594: 'George',
  595: 'Mrzysztof',
  596: 'Petar',
  597: 'February',
  598: 'Goran',
  599: 'Curt',
  600: 'Baldur',
  601: 'Hans-erich',
  602: 'Karl-Hans',
  603: 'Charles',
  604: 'Kristian',
  605: 'Arnim',
  606: 'Frederik',
  607: 'Timm',
  608: 'Ricardo',
  609: 'Caramel',
  610: 'Alf',
  611: 'Janos',
  612: 'Stefan',
  613: 'Carl-heinz',
  614: 'Juan',
  615: 'Kunibert',
  616: 'Gundolf',
  617: 'Julian',
  618: 'Aribert',
  619: 'Hakan',
  620: 'Stanislav',
  621: 'Alexey',
  622: 'Tibor',
  623: 'Heinz-Willi',
  624: 'Mahmut',
  625: 'Henning',
  626: 'Spar',
  627: 'Ramon',
  628: 'Ante',
  629: 'Adem',
  630: 'Hans-gerhard',
  631: 'Dariusz',
  632: 'Urban',
  633: 'Pedro',
  634: 'Hans-Uwe',
  635: 'Gaetano',
  636: 'Hans- ludwig',
  637: 'Burkard',
  638: 'Hans-dietrich',
  639: 'Hans- friedrich',
  640: 'Yaroslav',
  641: 'Hans- karl',
  642: 'Wolfgang',
  643: 'Heinz-Günther',
  644: 'Henner',
  645: 'Tomislav',
  646: 'Vitus',
  647: 'Turtle',
  648: 'Sergei',
  649: 'Freddy',
  650: 'Gary',
  651: 'Salih',
  652: 'Holland',
  653: 'José',
  654: 'Dittmar',
  655: 'Recep',
  656: 'Jessica',
  657: 'Stevan',
  658: 'Peter',
  659: 'Goddess',
  660: 'Heinz-Gerd',
  661: 'Tomas',
  662: 'Justus',
  663: 'Vito',
  664: 'Ryszard',
  665: 'Evangelos',
  666: 'Dusan',
  667: 'Anatoly',
  668: 'Alien',
  669: 'Sami',
  670: 'Zoltan',
  671: 'Cengiz',
  672: 'Raik',
  673: 'Jean',
  674: 'Sigmar',
  675: 'Leszek',
  676: 'Leon',
  677: 'Cemal',
  678: 'Sylvio',
  679: 'Hans-Albert',
  680: 'Eckhart',
  681: 'Willow',
  682: 'Frank-Michael',
  683: 'Willfried',
  684: 'Leonard',
  685: 'Peter',
  686: 'Peter',
  687: 'Franjo',
  688: 'Alien',
  689: 'Karl-Dieter',
  690: 'Brian',
  691: 'Anselm',
  692: 'Fedor',
  693: 'Slavko',
  694: 'Calogero',
  695: 'Alban',
  696: 'Casimir',
  697: 'Vassilios',
  698: 'Emmerich',
  699: 'Peter',
  700: 'Danilo',
  701: 'Pawel',
  702: 'Harold',
  703: 'Hans-rudolf',
  704: 'Tilmann',
  705: 'Joseph hermann',
  706: 'Vadim',
  707: 'Theodoros',
  708: 'Tony',
  709: 'Miguel',
  710: 'Mikhail',
  711: 'Klaus-D.',
  712: 'Donald',
  713: 'Cosimo',
  714: 'Peter',
  715: 'Frithjof',
  716: 'Slobodan',
  717: 'Robby',
  718: 'Cornelius',
  719: 'Ilya',
  720: 'Jochem',
  721: 'Leander',
  722: 'Ilija',
  723: 'Alberto',
  724: 'Tomasz',
  725: 'Peter',
  726: 'Jean',
  727: 'Theobald',
  728: 'Attila',
  729: 'Peter',
  730: 'Louis',
  731: 'Heinz wilhelm',
  732: 'Witold',
  733: 'Jobst',
  734: 'Ernst-Otto',
  735: 'Erudogan',
  736: 'Karl-Georg',
  737: 'Eberhardt',
  738: 'Hans-Rainer',
  739: 'Traugott',
  740: 'Peter',
  741: 'Burckhard',
  742: 'Anthony',
  743: 'Hans-Theo',
  744: 'Paul-Gerhard',
  745: 'Reimar',
  746: 'Jonathan',
  747: 'Anastasios',
  748: 'Squidward',
  749: 'Joseph',
  750: 'Alessandro',
  751: 'Enver',
  752: 'Romuald',
  753: 'Hans-Bernd',
  754: 'Drago',
  755: 'Hans-H.',
  756: 'Aleksandr',
  757: 'Alexandre',
  758: 'Augustine',
  759: 'Thies',
  760: 'Jan-Peter',
  761: 'Frederic',
  762: 'Sergio',
  763: 'Heinz-otto',
  764: 'Gerard',
  765: 'Maurice',
  766: 'Karl-August',
  767: 'Friedhold',
  768: 'Denny',
  769: 'Balthazar',
  770: 'Samir',
  771: 'Bayram',
  772: 'Jaroslav',
  773: 'Filippo',
  774: 'Celeb',
  775: 'Mato',
  776: 'Joseph',
  777: 'Jiri',
  778: 'Peter',
  779: 'Aziz',
  780: 'Zeki',
  781: 'Bartholomew',
  782: 'Leonardo',
  783: 'Isidore',
  784: 'Hansgeorg',
  785: 'Sarah',
  786: 'Muzaffer',
  787: 'Claas',
  788: 'Nick',
  789: 'Sandor',
  790: 'Thank god',
  791: 'Jann',
  792: 'Arif',
  793: 'Karl-Werner',
  794: 'Horst-Günter',
  795: 'Erdal',
  796: 'Stefano',
  797: 'Marijuana',
  798: 'Saban',
  799: 'Mariusz',
  800: 'Kenan',
  801: 'Daniele',
  802: 'Urs',
  803: 'Ercan',
  804: 'Slavomir',
  805: 'Remo',
  806: 'Amir',
  807: 'Zlatko',
  808: 'Jorge',
  809: 'Alfredo',
  810: 'Iliad',
  811: 'Celeb',
  812: 'Sabri',
  813: 'Lauren',
  814: 'Maurizio',
  815: 'Diether',
  816: 'Anatol',
  817: 'Linus',
  818: 'Edelbert',
  819: 'Bernt',
  820: 'Darko',
  821: 'Peter',
  822: 'Heinz Dieter',
  823: 'Dan',
  824: 'Thoralf',
  825: 'Johan',
  826: 'Phillip',
  827: 'Peter',
  828: 'Heinfried',
  829: 'Vain',
  830: 'Luciano',
  831: 'Carol',
  832: 'Bekir',
  833: 'Frederick',
  834: 'Peter',
  835: 'Giraffe',
  836: 'Antonino',
  837: 'Hans-wolfgang',
  838: 'Baptist',
  839: 'Hans joseph',
  840: 'Antonios',
  841: 'Wigbert',
  842: 'Philippe',
  843: 'Hans Joerg',
  844: 'Ben',
  845: 'Czeslaw',
  846: 'Kamil',
  847: 'Stefan',
  848: 'Micah',
  849: 'Jorgen',
  850: 'Frank-Peter',
  851: 'Jean',
  852: 'Roc',
  853: 'Geert',
  854: 'Gabor',
  855: 'Folker',
  856: 'Vlado',
  857: 'Corbinian',
  858: 'Ferenc',
  859: 'Ekrem',
  860: 'Stavros',
  861: 'Renato',
  862: 'Raymund',
  863: 'Celal',
  864: 'Vincent',
  865: 'Mikhail',
  866: 'Ingbert',
  867: 'Carol',
  868: 'Andres',
  869: 'Nicholas',
  870: 'Peter',
  871: 'Hans-adolf',
  872: 'Klaus-werner',
  873: 'Petros',
  874: 'Alexandros',
  875: 'Property',
  876: 'Peterin',
  877: 'Necati',
  878: 'Leif',
  879: 'Heinz',
  880: 'Massimo',
  881: 'Kenneth',
  882: 'Dogan',
  883: 'New Years Eve',
  884: 'Peter',
  885: 'Maxim',
  886: 'Peter',
  887: 'Sarah',
  888: 'Jean',
  889: 'Sedat',
  890: 'Mahmoud',
  891: 'Goddess',
  892: 'Gereon',
  893: 'Hans-Eberhard',
  894: 'Alan',
  895: 'Reza',
  896: 'Folkert',
  897: 'Ehrhard',
  898: 'Emin',
  899: 'Apostolos',
  900: 'Heinz walter',
  901: 'Utz',
  902: 'Veli',
  903: 'Nicholas',
  904: 'Klaas',
  905: 'H.-Dieter',
  906: 'Rudolph',
  907: 'Serge',
  908: 'Jovan',
  909: 'Abdul',
  910: 'Kazim',
  911: 'Klaus-michael',
  912: 'Alfonso',
  913: 'Alek',
  914: 'Roger',
  915: 'Rouven',
  916: 'Hans-detlef',
  917: 'Berend',
  918: 'Tassilo',
  919: 'Sukru',
  920: 'Gunar',
  921: 'Vinko',
  922: 'Ruben',
  923: 'Ron',
  924: 'Andrey',
  925: 'Aldo',
  926: 'Paulo',
  927: 'Peter-michael',
  928: 'Heimo',
  929: 'Ernest',
  930: 'Anto',
  931: 'Woldemar',
  932: 'Selim',
  933: 'Dursun',
  934: 'Engine',
  935: 'Abraham',
  936: 'Peter',
  937: 'Mattias',
  938: 'Janko',
  939: 'Ekkehart',
  940: 'Bela',
  941: 'Alexey',
  942: 'New Years Eve',
  943: 'Janus',
  944: 'Ilhan',
  945: 'Elias',
  946: 'Giorgio',
  947: 'Dinosaur',
  948: 'Pero',
  949: 'Muharrem',
  950: 'Mesut',
  951: 'Lorenzo',
  952: 'Thorben',
  953: 'Rigo',
  954: 'Oscar',
  955: 'Gino',
  956: 'Matteo',
  957: 'Peter',
  958: 'Francis',
  959: 'Zdravko',
  960: 'Reginald',
  961: 'Mehdi',
  962: 'Carol',
  963: 'Cetin',
  964: 'Milos',
  965: 'Friedo',
  966: 'Nuri',
  967: 'Nurettin',
  968: 'Eggert',
  969: 'Ralf-Dieter',
  970: 'Pirmin',
  971: 'Bernd-Dieter',
  972: 'Zenon',
  973: 'Senol',
  974: 'Klaus-Günter',
  975: 'Cemil',
  976: 'Abram',
  977: 'Sebastiano',
  978: 'Mijo',
  979: 'Grzegorz',
  980: 'Stefan',
  981: 'Arnfried',
  982: 'Ada',
  983: 'Sarah',
  984: 'Riza',
  985: 'Miodrag',
  986: 'Faruk',
  987: 'Imre',
  988: 'Ian',
  989: 'Marten',
  990: 'Hans-Hinrich',
  991: 'Abbas',
  992: 'Tillmann',
  993: 'Marvin',
  994: 'Ignatz',
  995: 'Kay-Uwe',
  996: 'Alexei',
  997: 'Willibert',
  998: 'Vittorio',
  999: 'Fredo'}}


#NOUN LIST 186 PAIRS
noun_pairs = {
    "Artz": "Arztin",
    "Vater": "Mutter",
    "Stiefsohn": "Stieftochter",
    "Herzog": "Herzogin",
    "Zauberer": "Hexe",
    "Baron": "Baronin",
    "Prinz": "Prinzessin",
    "Schwiegersohn": "Schwiegersohn",
    "Freund": "Freundin ",
    "Onkel": "Tante ",
    "Jager": "Jagerin",
    "Meister": "Herrin",
    "Grossvater": "Oma",
    "Madchen": "Junge",
    "Mann": "Frau",
    "Priester": "Priesterin",
    "Monch": "Nonne",
    "Gott": " Gottin",
    "Steward": "Sewardess",
    "Mord": "Morderin",
    "Kellner": "Kellnerin",
    "Kaiser": "Kaiserin",
    "Konig": "Konigin",
    "Ehemann":"Frau",
    "Stiefvater": "Stiefmutter",
    "Sohn": "Tochter",
    "Geschaftsmann": "Geschaftsfrau",
    "Witwer": "Witwe",
    "Schauspieler": "Schauspielerin",
    "Krankenpfleger": "Krankenschwester",
    "Schreiber": "Schreiberin",
    "Patensohn": "Patentochter",
    "Pate": "Patin",
    "Techniker": "Technikerin",
    "Ingenieur": "Ingenieurin ",
    "Fahrer": "Fahrerin",
    "Chirurg": "Chirurgin",
    "Mechaniker": "Mechanikerin",
    "Pionier":"Pionierin",
    "Regisseur":"Regisseurin", #DIRECTOR
    "Karikaturist":"Karikaturistin",#cartoonist
    "Musiker":"Musikerin",
    "Physiker":"Physikerin",
    "Grunder":"Grunderin", #FOUNDER
    "Muslim":"Muslimin", #RELIGIONS
    "Christ":"Christin",
    "Jude":"Judin",
    "Atheist":"Atheistin",
    "Botaniker":"Botanikerin",
    "Chemiker":"Chemikerin",
    "Historiker":"Historikerin",
    "Burger":"Burgerin",
    "Gegner":"Gegnerin",
    "Auslander":"Auslanderin", #foreigner
    "Sanger":"Sangerin",
    "Dichter":"Dichterin", #poet
    "Teilnehmer":"Teilnehmerin", #particopant
    "Handwerker":"Handwerkerin", #artisan
    "Partisan":"Partisanin",
    "Soldat":"Soldatin",
    "Boss":"Bossin",
    "Chef":"Chefin", #military terms
    "Fuhrer":"Fuhrerin",
    "Kommandant":"Kommandantin",
    "Kapitan":"Kapitanin",
    "Sklave":"Sklavin",
    "Entertainer":"Entertainerin",
    "Prasident":"Prasidentin",
    "Skifahrer":"Skifahrerin",
    "Fussballer":"Fussballerin",
    "Cricketspieler":"Cricketspielerin",
    "Berater":"Beraterin",
    "Therapeut":"Therapeutin",
    "Psychologe":"Psychologin",
    "Kardiologe":"Kardiologin",
    "Dermatologe":"Dermatologin",
    "Zahnarzt":"Zahnarztin",
    "Praktiker":"Praktikerin",
    "Neurologe":"Neurologin",
    "Ernahrungsberater":"Ernahrungsberaterin",
    "Anasthesist":"Anasthesistin",
    "Apotheker":"Apothekerin",
    "Chirurg":"Chirurgin",
    "Entbindungspfleger":"Hebamme",
    "Audiologe":"Audiologin",
    "Augenoptiker":"Augenoptikerinnen",
    "Optiker":"Optikerin",
    "Tierarzte":"Tierarztinnen",
    "Podologen":"Podologinnen",
    "Endokrinologe":"Endokrinologin",
    "Geriater":"Geriater",
    "Internist":"Internistin",
    "Geburtshelfer":"Geburtshelferin",
    "Urologe":"Urologin",
    "Neurochirurg":"Neurochirurgin",
    "Statistiker":"Statistikerin",
    "Wissenschaftler":"Wissenschaftlerin",
    "Erfinder":"Erfinderin",
    "Urheber":"Urheberin",
    "Autor":"Autorin",
    "Schriftsteller":"Schriftstellerin",
    "Archaologe":"Archaologin",
    "Astronaut":"Astronautin",
    "Astronom":"Astronomin",
    "Biochemiker":"Biochemikerin",
    "Gynakologe":"Gynakologin",
    "Okologe":"Okologin",
    "Förster":"Försterin",
    "Aufseher":"Aufseherin",
    "Geograph":"Geographin",
    "Naturforscher":"Naturforscherin",
    "Pathologe":"Pathologin",
    "Palaontologe":"Palaontologin",
    "Anthropologe":"Anthropologin",
    "Okonom":"Okonomin",
    "Historiker":"Historikerin",
    "Soziologe":"Soziologin",
    "Planer":"Planerin",
    "Klempner":"Klempnerin",
    "Schweisser":"Schweisserin",
    "Holzarbeiter":"Holzarbeiterinnen",
    "Arbeiter":"Arbeiterin",
    "Muller":"Mullerin",
    "Leibwachter":"Leibwachterin",
    "Polizist":"Polizistin",
    "Rechtsanwalt":"Rechtsanwaltin",
    "Stellvertreter":"Stellvertreterin",
    "Vertreter":"Vertreterin",
    "Verkaufer":"Verkauferin",
    "Verkaufer":"Verkauferin",
    "Aufseher":"Aufseherin",
    "Rektor":"Rektorin",
    "Pastor":"Pastorin",
    "Cousin":"Cousine",
    "Tanzer":"Tanzerin", #dancer
    "Dieb":"Diebin",
    "Ehepartner":"Ehepartnerin",
    "Illustrator":"Illustratorin",
    "Designer":"Designerin",
    "Jager":"Jagerin",
    "Angreifer":"Angreiferin",
    "Uberlebender":"Uberlebende",
    "Einwohner":"Einwohnerin",
    "Bewohner":"Bewohnerin",
    "Forscher":"Forscherin",
    "Programmierer":"Programmiererin",
    "Professor":"Professorin",
    "Dozent":"Dozentin",
    "Begleiter":"Begleiterin",
    "Besucher":"Besucherin",
    "Theologe":"Theologin",
    "Immigrant":"Immigrantin",
    "Farmer":"Farmerin",
    "Senator":"Senatorin",
    "Wanderer":"Wanderin",
    "Kurator":"Kuratorin",
    "Wachter":"Wachterin",
    "Treuhander":"Treuhanderin",
    "Huter":"Huterin",
    "Spion":"Spionin",
    "Klager":"Klagerin",
    "Antragsteller":"Antragstellerin",
    "Morder":"Morderin",
    "Dieb":"Diebin",
    "Pazifist":"Pazifistin",
    "Metzger":"Metzgerin",
    "Verfuhrer":"Verfihrerin",
    "Kanzler":"Kanzlerin",
    "Gewinner":"Gewinnerin",
    "Einsiedler":"Einsiedlerin",
    "Prostituierter":"Prostituierte",
    "Gastronom":"Gastronomin",
    "Laufer":"Lauferin",
    "Bote":"Botin",
    "Journalist":"Journalistin",
    "Publizist":"Publizistin",
    "Vegetarier":"Vegetarierin",
    "Schwimmer":"Schwimmerin",
    "Uhrmacher":"Uhrmacherin",
    "Lugner":"Lugnerin",
    "Schwindler":"Schwindlerin",
    "Stripper":"Stripperin",
    "Maler":"Malerin",
    "Richter":"Richterin",
    "Experte":"Expertin",
    "Lowe":"Lowin", #animals lion
    "Kater":"Katze"
}

def remove_punc(my_str):
  punctuations = '''!()-[]{};:\,<>./?@#$%^&*_~'''
  no_punct = ""
  for char in my_str:
   if char not in punctuations:
       no_punct = no_punct + char
  return no_punct 


def preserve_capitalization(original, transformed):
  if original[0].isupper():
    transformed = transformed.capitalize()
    return transformed
  else:
    transformed=transformed.lower()
    return transformed
#CHECK IF PRECEEDING WORD NEEDS TO BE REPLACED
def preceding_word(n):
  n = n.lower()
  
  if n=="der": #nominative case
    n="die"
    return n
  elif n=="ein":
    n="eine"
    return n
  elif n=="mein": #possessive adjectives in nominative case
    n="meine"
    return n
  elif n=="dein":
    n="deine"
    return n
  elif n=="mein":
    n="meine"
    return n
  elif n=="sein":
    n="seine"
    return n
  elif n=="unser":
    n="unsere"
    return n
  elif n=="euer":
    n="eure"
    return n
  elif n=="ihr":
    n="ihre"
    return n
  elif n=="den": #accusative case
    n="die"
  else:
    return n


def nounswap(input,noun_pairs): #REPLACE M WITH F NOUN
  i=remove_punc(input)
  new = ''.join(' ' if c in '.!' else c for c in i)
  t = new.split()
  for word in t:
    print(word)
    if word in noun_pairs.keys(): #check for nouns
      #check for previous word in list, lowercase it, check and replace from dict
     
       n = t[t.index(word) - 1]
       original_n=n
       new=preceding_word(n)  
       print(new)    
       new=preserve_capitalization(original_n,new)
       i=i.replace(n,str(new))
       print(i)
       print(noun_pairs[word])
       new = i.replace(word,str(noun_pairs[word]))  
  
  return new

#PERSONAL PRONOUNS
personalp={
    "er":"sie", #nominative
    "Er":"Sie", #nominative
    "ihr":"sie",
    "Ihr":"Sie",
    "ihn":"sie", #acc
    "Ihn":"Sie", 
    "Ihm":"Ihr",
    "ihm":"ihr", #dative
}
def replace_pers(inp,personalp):
  i=remove_punc(inp)
  i = ''.join(' ' if c in '.!' else c for c in i)
  t = i.split()
  for word in t:
    print(word)
    if word in personalp.keys():
      i=i.replace(word,str(personalp[word]))
      #print(i)     
  return i
#inp="Er is weg."
#replace_pers(inp,personalp)


#get list of names
f=list(names[1].values())
m=list(names[2].values())

def findname(sent,malenames,femalenames):
  sent=remove_punc(sent)
  sent = ''.join(' ' if c in '.!' else c for c in sent)
  t = sent.split()
  for word in t:
    if word in malenames:
      w=word
      #text=text.replace(word,w)
      return w
    elif word in femalenames:
      w=word
      return w
    else:
      print(" ")
def swapname(name,names):
  for n in names[2].values():
    if name in m:
      new=random.choice([i for i in names[1].values()])
      return new
    else:return name


def newnamerep(m,f,inp,names):
  n=findname(inp,m,f)
  sent=remove_punc(inp)
  sent = ''.join(' ' if c in '.!' else c for c in sent)
  t = sent.split()
  for word in t:
    if word in m and n!='None':
      #print(word)
      if n!=None:
        fname=swapname(word,names)
        sent=sent.replace(word,str(fname))
        
      else:
        sent=sent
    else:
      sent= sent

    return sent







def german_nouns(text, noun_pairs):
    t1=nounswap(text,noun_pairs)
    t2=replace_pers(t1,personalp)
    print(t2)
    t3=newnamerep(m,f,t2,names)
    
    return t3 


class GermanGenderSwap(SentenceOperation):

    tasks = [
        TaskType.TEXT_CLASSIFICATION,
        TaskType.TEXT_TO_TEXT_GENERATION
    ]
    languages = ["de"]

    def __init__(
        self, seed=0, max_outputs=1):
        super().__init__(seed, max_outputs)
        self.noun_pairs = noun_pairs

        
    def generate(self, sentence: str):
        return [german_nouns(sentence, self.noun_pairs)]
    
 
if __name__ == '__main__':
    import json
    from TestRunner import convert_to_snake_case
    tf = GermanGenderSwap(max_outputs=1)
    test_cases = []
    for sentence in ["der Artz arbeitet.",
                     "Der Ingenieur geht.",
                     "Er ist mein Vater.",
                     "John ist hier",
                     "Er ist weg"]:
        test_cases.append({
            "class": tf.name(),
            "inputs": {"sentence": sentence}, "outputs": [{"sentence": o} for o in tf.generate(sentence)]}
        )
    json_file = {"type": convert_to_snake_case(tf.name()), "test_cases": test_cases}
    print(json.dumps(json_file, indent=2))
