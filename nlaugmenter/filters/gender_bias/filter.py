...
class GenderBiasFilter(SentenceOperation):
    ...
    def flag_sentences(
        ...
        for sentence in sentences:
            ...
            intersection_feminine = set(
                processed_sentence.split()
            ).intersection(set(joint_feminine))
            ...
            intersection_masculine = set(
                processed_sentence.split()
            ).intersection(set(joint_masculine))
    ...
    def count(self, group_flag, flagged_corpus):
        ...
        flags_count = len(
            [flag for flag in flagged_corpus if flag.get(group_flag) is True]
        )
        return flags_count
    ...
    def filter(self, sentences: []) -> bool:
        ...
        feminine_percentage = (
            100 * float(feminine_count) / float(len(sentences))
        )
        masculine_percentage = (
            100 * float(masculine_count) / float(len(sentences))
        )
