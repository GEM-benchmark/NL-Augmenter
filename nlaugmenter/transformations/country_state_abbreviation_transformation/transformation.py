...
class CountryStateAbbreviation(SentenceOperation):
    ...
    def build_mapping(self):
        ...
        country_abbr = {
            country["name"]: country["abbr"] for country in abbr_json
        }
        country_exp = {
            country["abbr"]: country["name"] for country in abbr_json
        }
        state_abbr = {}
        state_exp = {}
        ...
        if self.country_filter:
            country = next(
                (x for x in abbr_json if x["abbr"] == self.country_filter),
                None,
            )
            if country:
    ...
    def country_state_abbreviation(self, text):
        ...
        state_regex = re.compile(
            r"(^|\s+)(" + state_pattern + r")(\s+|\?|!|$|\.|,)"
        )
        perturbed_text = state_regex.sub(
            lambda y: y[1]
            + self.dict_value_helper(state_mapping, y[2])
            + y[3],
            perturbed_text,
        )
