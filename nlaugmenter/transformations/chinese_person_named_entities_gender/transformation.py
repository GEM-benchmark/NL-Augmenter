...
def chinese_person_named_entities_gender(
    text, prob, seed, max_outputs, gender_change, chinese_names_data
):
    random.seed(seed)
...
def load_chinese_names_data():
    ...
    with open(
        os.path.join(dirname, "Chinese_Names_Corpus_Gender（120W）.txt"),
        mode="r",
    ) as file:
...
class ChinesePersonNamedEntitiesAndGender(SentenceOperation):
    ...
    def generate(
        self, sentence: str, seed: int = 0, gender_change: bool = True
    ):
