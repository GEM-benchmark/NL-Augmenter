...
def create_token_and_tag_seq(
    token_seq, tag_seq, b_tag_index, i_tag_index, b_tag, i_tag, seed
):
    """
    ...
    if perturb_type == "pre":
        token_seq, tag_seq = add_location_prefix(
            token_seq, tag_seq, phrase, b_tag_index, b_tag, i_tag
        )
    if perturb_type == "post":
        token_seq, tag_seq = add_location_postfix(
            token_seq, tag_seq, phrase, i_tag_index, i_tag
        )
...
def add_location_prefix(token_seq, tag_seq, phrase, b_tag_index, b_tag, i_tag):
    ...
    if len(word_list) == 1:  # if selected phrase contains single word
        token_seq.insert(
            b_tag_index, phrase
        )  # put phrase in token_seq at index b_tag_index
        tag_seq[b_tag_index] = i_tag  # replace B-LOC with I-LOC
        tag_seq.insert(
            b_tag_index, b_tag
        )  # put B-LOC in tag_seq at index b_tag_index
        return token_seq, tag_seq
...
def extract_tag_indexes(tag_seq, b_tag, i_tag):
    ...
    if b_tag_index == len(tag_seq) - 1:
        ...
        return (
            b_tag_index,
            i_tag_index,
        )  # case when B-LOC at the end of tag_sequence", no I-LOC
    if tag_seq[b_tag_index + 1] == "O":
        i_tag_index = (
            b_tag_index  # case when B-LOC in the middle but no "I-LOC"
        )
...
class LongerLocationNer(TaggingOperation):
    ...
    def generate(
        ...
        if b_tag in tag_seq:
            b_tag_index, i_tag_index = extract_tag_indexes(
                tag_seq, b_tag, i_tag
            )
            for _ in range(self.max_outputs):
                token_seq, tag_seq = create_token_and_tag_seq(
                    token_seq,
                    tag_seq,
                    b_tag_index,
                    i_tag_index,
