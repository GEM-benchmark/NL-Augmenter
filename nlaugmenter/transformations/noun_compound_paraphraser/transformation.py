    ...
    for k in compounds.keys():
        if k in L[0][0]:
            ...
            for line in L:
                for text in line:
                    n = [
                        text.replace(k, paraphrase[0])
                        for paraphrase in compounds[k]
                    ]
                    new.extend(n)
            L.append(new)
...
class NounCompoundParaphraser(SentenceOperation):
    ...
    def __init__(self, seed=0, max_paraphrases=1):
        ...
        with open(
            os.path.join(
                data_dir, "compound_paraphrases_semeval2013_task4.json"
            ),
            "r",
        ) as fd:
        ...
        for k in compound_paraphrases.keys():
            randomized_ties = sorted(
                compound_paraphrases[k],
                key=lambda i: (int(i[1]), random.random()),
                reverse=True,
            )
