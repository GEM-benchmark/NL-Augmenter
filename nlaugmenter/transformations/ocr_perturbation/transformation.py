...
class OcrPerturbation(SentenceOperation):
    ...
    def generate(self, sentence: str) -> List[str]:
        ...
        with PyTessBaseAPI(
            lang=tess_lang[self.language], psm=PSM.RAW_LINE, oem=OEM.LSTM_ONLY
        ) as ocr:
            ...
            image_generators = [
                self._get_image_generator(sent.text) for sent in doc.sents
            ]
    ...
    def _get_image_generator(self, sentence):
        ...
        fonts = [
            font
            for font in gen.fonts
            if not any(s in font.lower() for s in ["display"])
        ]
