...
from .constants import (
    abbreviated_currency_symbols_to_currency_name_dict,
    symbol_to_currency_name_dict,
)
...
def recognize_transform(word, prev_word, next_word):
    ...
    if recognized_as_power_of_ten(word, prev_word):
        ...
        (
            incomplete_date_recognized,
            new_incomplete_date_word,
        ) = recognized_as_incomplete_date(word)
        ...
        if datestring_recognized:
            ...
            words = (
                word[0] + recognize_transform(word[1:-1], " ", " ") + word[-1]
            )
            return words
...
def recognized_as_range_not_sticky(word, next_word):
    ...
    if (
        ...
        and stripe_index > -1
        and len(word[:stripe_index]) <= 3
        and len(word[stripe_index + 1 :]) <= 3
    ):
        ...
        return (
            bool(re.search(r"^\d*[-]?\d*$", first_part))
            and len(last_part) == 0
            and word[0].isnumeric()
        )
    else:
        return False
...
def recognized_as_date_word(word, prev_word, next_word):
    return (
        (prev_word.lower() in month_words or next_word.lower() in month_words)
        and word.isnumeric()
        and int(word) <= 31
    )
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            datetime_x = datetime.datetime.strptime(
                                x, "%y-%m-%d"
                            )
                            return True, datetime_x
                        except ValueError:
...
def recognized_as_year(x):
    ...
    checker = (
        min(
            [
                ...
                for character in after_assumed_year
            ]
            + [True]
        )
        and min(
            [
                character in string.punctuation
                for character in before_assumed_year
            ]
            + [True]
        )
        and year in possible_year_list
        and (len(year) <= 4)
        and year.isnumeric()
    )
    ...
    if checker:
        return (
            bool(re.compile(r".*([1-3][0-9]{3})").match(x))
            and len(set(x) - {"0"}) >= 3
        )
    else:
        return False
...
def recognized_as_currency_symbols(x):
    ...
    currency_abbreviations = list(
        abbreviated_currency_symbols_to_currency_name_dict.keys()
    )
    ...
    if not found_dot:
        ...
        if len(front_checker) > 0:
            front_checker = (
                front_checker[:-1]
                if (front_checker[-1] in [".", ","])
                else front_checker
            )
        else:
            back_checker = (
                back_checker[:-1]
                if (back_checker[-1] in [".", ","])
                else back_checker
            )

    if front_checker in currency_symbols:
        other_end_non_numeric = x[begin_digit_index:][
            end_digit_index - (len(x[:begin_digit_index])) :
        ]
        return (
            other_end_non_numeric in currency_symbols
            or other_end_non_numeric in currency_abbreviations
            or len(other_end_non_numeric) == 0
        )
...
def recognized_as_sticky_numbers(x):
    ...
    return (
        bool(re.search(r"^\d*[.,]?\d*$", first_part))
        and (
            last_part in ["st", "nd", "rd", "th", "%"]
            or not re.search(r"\d", last_part)
        )
        and x[0].isnumeric()
    )
...
def recognized_as_sticky_range(x):
    ...
    return (
        bool(re.search(r"^\d*[-]?\d*$", first_part))
        and len(last_part) > 0
        and not re.search(r"\d", last_part)
        and x[0].isnumeric()
    )
...
def recognized_as_general_numbers(x):
    return (
        x.replace(",", "").replace(".", "").isnumeric() and x[-1].isnumeric()
    )
...
def recognized_as_fraction(x):
    ...
    return (
        len(x[:divider_index]) <= 2
        and len(x[divider_index + 1 :]) <= 2
...
def datestring_to_words(x):
    words = (
        "the "
        + num2words(x.day, to="ordinal")
        + " of "
        + calendar.month_name[x.month].lower()
        + " "
        + year_to_words(str(x.year))
    )
    return words
...
def incompelete_date_to_words(x):
    words = (
        calendar.month_name[x.month].lower() + " " + year_to_words(str(x.year))
    )
    return words
...
def currency_to_words(x):
    ...
    if front_checker in currency_symbols:  # $300
        if x.find(".") > -1:
            ...
            currency = str.lower(
                symbol_to_currency_name_dict[
                    re.sub("[.]", "", x[:begin_digit_index])
                ]
            )
            if (
                len(number[number.index(".") + 1 :]) > 0
                and len(number[: number.index(".")]) == 0
            ):
                if int(number[number.index(".") + 1 :]) > 1:
                    words = (
                        num2words(number[number.index(".") + 1 :])
                        + " "
                        + currency
                        + " cents"
                    )
        ...
        if (
            number.find(".") > -1
            and len(number[number.index(".") + 1 :]) > 0
            and len(number[: number.index(".")]) == 0
        ):
            if int(number[number.index(".") + 1 :]) > 1:
                words = (
...
def sticky_numbers_to_words(x):
    ...
    if last_part in ["st", "nd", "rd", "th"]:
        ...
        words = (
            num2words(first_part, to="cardinal") + " " + last_part
            if len(last_part) > 0
            else num2words(first_part, to="cardinal")
        )
    return words
...
def sticky_range_to_words(x):
    ...
    words = (
        num2words(first_part[: first_part.index("-")])
        + " to "
        + num2words(first_part[first_part.index("-") + 1 :])
        + " "
        + last_part
    )
    return words
...
def math_formula_equality_to_words(x):
    ...
    end_equality_sign_index_in_word = x.index(equality_sign) + len(
        equality_sign
    )
    ...
    end_digit_index = (
        len(after_equal) - re.search(r"\d", after_equal[::-1]).start()
    )

    first_part = after_equal[begin_digit_index:end_digit_index]
    last_part = after_equal[end_digit_index:]
    after_equal = (
        recognize_transform(first_part, "", "")
        + " "
        + recognize_transform(last_part, "", "")
    )

    words = (
        recognize_transform(before_equal, "", "")
        + " "
        + equality_sign
        + " "
        + after_equal
...
def numeric_beside_end_bracket_to_words(x):
    end_digit_index = len(x) - re.search(r"\d", x[::-1]).start()
    words = (
        recognize_transform(x[:end_digit_index], " ", " ")
        + " "
        + x[end_digit_index:]
    )
    return words
...
def natural_log_to_words(x):
    ...
    words = (
        "e ( "
        + recognize_transform(x[2:end_digit_index], " ", " ")
        + " "
        + x[end_digit_index:]
    )
    return words
