        ...
        if word.lower() not in l_df.index:
            ...
            if rand_prob < prob_mix:
                ...
                if not word[0].isupper():  # lower case
                    ...
                    perturbed_word = l_df.loc[
                        plain_word, mlt_tgt_lang
                    ].capitalize()
...
class MultilingualLexiconPerturbation(SentenceOperation):
    ...
    def __init__(
        self, seed=0, prob_mix=0.5, mlt_src_lang="en", mlt_tgt_lang="zh"
    ):
        super().__init__(seed)
        ...
        if mlt_src_lang not in self.supported_languages:
            raise ValueError(
                f'Invalid `mlt_src_lang` value "{mlt_src_lang}". Supported languages: {self.supported_languages}'
            )

        if mlt_tgt_lang not in self.supported_languages:
            raise ValueError(
                f'Invalid `mlt_tgt_lang` value "{mlt_tgt_lang}". Supported languages: {self.supported_languages}'
            )

        self.lexicon_df = pd.read_pickle(
            f"{FOLDER_PATH}/multilingual_lexicon_uncased.xz"
        )
    ...
    def generate(self, sentence: str):
        pertubed_sentence = perturb_sentence(
            lexicon_df=self.lexicon_df,
            text=sentence,
            prob_mix=self.prob_mix,
            mlt_src_lang=self.mlt_src_lang,
            mlt_tgt_lang=self.mlt_tgt_lang,
            seed=self.seed,
        )
