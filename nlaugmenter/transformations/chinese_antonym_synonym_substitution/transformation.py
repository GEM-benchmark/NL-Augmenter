...
def chinese_antonym_substitution(text, prob, chinese_antonym_data):
    ...
    for perturb_word in text:
        ...
        if perturb_word not in chinese_antonym_data.keys():
            print(
                "Word to be perturbed is not in the Chinese Antonym database"
            )
        else:
...
class ChineseAntonymAndSynonymSubtitution(SentenceOperation):
    ...
    def generate(self, sentence: str, config: str = "synonym"):
        ...
        if config == "synonym":
            smw = Similarword(
                create_num=self.max_outputs + 1, change_rate=self.prob
            )
