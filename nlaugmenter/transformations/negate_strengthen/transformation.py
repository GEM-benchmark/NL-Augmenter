...
class NegateStrengthen(SentenceAndTargetOperation):
    ...
    def negation_rules(self, tgx, table, pos=None, method=[], curr_try=0):
        ...
        if curr_try > self.num_tries:
            ...
            if self.wnl.lemmatize(text[tgx - 1], "v") in MODAL_TYPE_LEMMA or (
                pos[tgx - 1] in ["MD"]
            ):
            ...
            # if word before target is noun type
                ...
                text[tgx - 1] = "<edit>did not</edit> " + self.wnl.lemmatize(
                    text[tgx - 1], "v"
                )
            # if word is last word
            elif tgx >= len(text):
            ...
            # if word before target is AUX | if word after target is IN/TO
                ...
                self.wnl.lemmatize(text[tgx - 2], "v") in MODAL_TYPE_LEMMA
                or (pos[tgx - 2] in ["MD"])
                or (pos[tgx] in ["IN", "TO"])
            ):
    ...
    def improve_negation_flow(self, text, edit_id, edit_method):
        ...
        if conjugate_en(verb=word, tense="past", number="singular") == word:
            ...
            conjugate_en(verb=word, tense="present", number="singular") == word
        ):
            tense = "present"
        elif (
            conjugate_en(verb=word, tense="participle", number="singular")
            == word
        ):
            tense = "participle"
        ...
        if len(ant) > 0:
            ...
            if tense is not None:
                edit_word = conjugate_en(
                    verb=edit_word, tense=tense, number="singular"
                )
            else:
                if self.verbose:
                    print(
                        'Caution: Unsure of tense of "{}" to "{}"...'.format(
                            word, edit_word
                        )
                    )
    ...
    def select_edit(self, edits_dict):
        ...
        if edits_dict["alt_text"] is not None:
            ...
            alt_word = self.wnl.lemmatize(
                re.sub("(\\<(\\/)*edit\\>)", "", alt_word)
            )
        else:
            alt_word = None
        not_word = edits_dict["edit_text"][edit_id - 1]
        orig_word = self.wnl.lemmatize(
            re.sub("(\\<(\\/)*edit\\>)", "", not_word)
        )
        not_word = re.sub("(\\<(\\/)*edit\\>)", "", not_word)

        if self.verbose:
            print(
                "orig_word: {} | not_word: {} | alt_word: {}".format(
                    orig_word, not_word, alt_word
                )
            )
    ...
    def text_to_negated_edits(self, _text, get_roots=None):
        ...
        if get_roots is None:
            get_roots = table[
                ([True if t[0:2] == "VB" else False for t in table["pos_tag"]])
                & (table["dep"] == "ROOT")
            ].index
        if self.verbose:
        ...
        for jx, root in enumerate(get_roots):
            ...
            if table.loc[root, "lemma"] in MODAL_TYPE_LEMMA or (
                table.loc[root, "pos_tag"] in ["MD"]
            ):
                if (
                    len(
                        table[
                            (table["dep"].isin(["acomp", "attr"]))
                            & (table["head_id"] == root)
                        ]
                    )
                    > 0
            ...
            if None not in method and len(method) > 0:
                alt_text, alt_word = self.improve_negation_flow(
                    _text.split(" "), edit_id, method
                )
                # if method is nor format
                if alt_text is not None:
                    if method[-1] == "JJ_1_2":
                        alt_text, alt_word_2 = self.improve_negation_flow(
                            alt_text.copy(), edit_id + 2, method
                        )
    ...
    def text_to_stronger_edits(self, _text):
        ...
        table = self.text_to_spacy_table_info(_text)
        get_roots = table[
            table["lemma"].isin(MODAL_STRENGTHEN_DICT.keys())
        ].index
        ...
        for jx, root in enumerate(get_roots):
            ...
            if table.loc[root + 1, "lemma"] == "be":
                ...
                if table.loc[root + 2, "lemma"] == "be":
                    ...
                    text[root + 1] = ""
                    method = ["MOD_3_1"]
            elif (table.loc[root, "pos_tag"] == "MD") and (
                table.loc[root + 1, "pos_tag"] == "RB"
            ):
                text[root] = (
                    "<edit>" + MODAL_STRENGTHEN_DICT[text[root]] + "</edit>"
                )
                text[root + 1] = ""
                method = ["MOD_4_1"]
            else:
    ...
    def generate(self, sentence: Union[dict, str], target: str):
        ...
        perturbed_items = list(
            set(
                [
                    (perturbed_sentences[i], perturbed_target)
