...
class ChangeGenderCultureDiverseNameTwoWay:
    ...
    def apply(
        ...
        for x in ents:
            ...
            if name in self.name_all:
                ...
                for new_name in new_names:
                    ...
                    ret_t.append(
                        re.sub(r"\b%s\b" % re.escape(name), new_name, tar)
                    )
        ...
        if len(ret_s) > max_output:
            ...
            return (
                [ret_s[idx] for idx in idxs],
                [ret_t[idx] for idx in idxs],
                [ret_m[idx] for idx in idxs],
            )
        else:
...
class GenderCultureDiverseNameTwoWay(SentenceAndTargetOperation):
    ...
    def generate(
        ...
        return [
            (perturbed_source[idx], perturbed_target[idx])
            for idx in range(len(perturbed_source))
        ]
