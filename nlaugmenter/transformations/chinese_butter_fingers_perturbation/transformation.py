...
def chinese_butter_finger(
    ...
    for _ in range(max_outputs):
        ...
        if word_level_perturb:
            words_and_similar_word_dict_list = get_words_with_similar_pinyin(
                output,
                rare_word_prob,
                chinese_character_database,
                common_chinese_character_database,
            ...
            for dict in words_and_similar_word_dict_list:
                ...
                if random.random() <= prob and len(similar_pinyins_words) != 0:
                    ...
                    new_chinese_character = random.choice(
            ...
            for chinese_character in text:
                similar_pinyins = get_characters_with_similar_pinyin(
                    chinese_character,
                    rare_word_prob,
                    chinese_character_database,
                    common_chinese_character_database,
        ...
        if butter_text not in perturbed_texts:
            ...
            perturbed_texts.append(
                "Similar output exists in perturbed outputs"
            )
...
def get_characters_with_similar_pinyin(
    ...
    if random.random() <= rare_word_prob:
        chars_with_similar_pinyin = retrieve_from_database(
            chinese_character,
            chars_with_similar_pinyin,
            chinese_character_database,
            consider_tone,
...
def get_words_with_similar_pinyin(
    ...
    for original_word in text:
        ...
        similar_word_pinyin_list = get_similar_word_pinyin_list(
            chinese_character_database,
            chinese_words_database,
            common_chinese_character_database,
            consider_tone,
    ...
    if original_word_len > 1:
        for i in range(len(chinese_words_database)):
            ...
            if len(candidate_word) == original_word_len:
                ...
                word_pinyins_flatten = [
                    item for pinyin in word_pinyins for item in pinyin
                ]
                word_pinyins_string = "".join(word_pinyins_flatten)

                if not consider_tone:
                    original_word_pinyins_string = u.unidecode(
                        original_word_pinyins_string
                    )
                    word_pinyins_string = u.unidecode(word_pinyins_string)

                if (original_word_pinyins_string == word_pinyins_string) and (
                    original_word != candidate_word
                ):
...
def retrieve_from_database(
    ...
    for char_dict in chinese_character_database:
        if consider_tone:
            if (pinyin_for_char_to_be_perturbed == char_dict["pinyin"]) and (
                chinese_character != char_dict["word"]
            ):
                chars_with_similar_pinyin += char_dict["word"]
        else:
            if u.unidecode(pinyin_for_char_to_be_perturbed) == u.unidecode(
                char_dict["pinyin"]
            ) and (chinese_character != char_dict["word"]):
                chars_with_similar_pinyin += char_dict["word"]
    return chars_with_similar_pinyin
...
def load_chinese_words_data():
    ...
    words_list = []
    with open(
        os.path.join(dirname, "thuocl/thuocl_all.txt"), mode="r"
    ) as file:
...
class ChineseButterFingersPerturbation(SentenceOperation):
    ...
    def __init__(
        ...
        self.chinese_character_database = (
            load_chinese_character_data_into_memory()
        )
        self.common_chinese_character_database = (
            load_common_chinese_character_data_into_memory()
        )
...
if __name__ == "__main__":
    ...
    output = generator.generate(
        "本意是指词的起源义，即词的最初意义。引申义是由词的本意引申出来的并经过推演发展而产生的意义"
    )
    print(output)
